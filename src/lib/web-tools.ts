...
/**
 * Adresse privée, locale ou de métadonnées cloud ?
 *
 * `fetch_page` reçoit une URL choisie par le modèle, donc influençable par
 * l'utilisateur. Sans ce filtre, on pouvait faire interroger au serveur ses
 * propres services internes ou l'endpoint de métadonnées de l'hébergeur
 * (169.254.169.254), et récupérer le contenu dans la réponse du chat.
 */
function isBlockedHost(hostname: string): boolean {
  ...
  if (v4) {
    ...
    if (a === 127 || a === 10 || a === 0) return true;              // loopback, privé, non routable
  ...
  // IPv6 : loopback, unique-local (fc00::/7) et link-local (fe80::/10)
  if (/^(fc|fd|fe8|fe9|fea|feb)/i.test(host) && host.includes(':')) return true;

  return false;
}

/** Valide une URL avant toute requête sortante déclenchée par le modèle. */
function assertFetchableUrl(raw: string): URL {
  ...
  if (isBlockedHost(parsed.hostname)) {
    throw new Error(`Hôte interne refusé : ${parsed.hostname}`);
  }
  return parsed;
}
...
export async function fetchPageText(url: string): Promise<string> {
  try {
    ...
    console.warn(`[FETCH PAGE] refusé : ${(e as Error).message}`);
    return `Impossible de lire cette adresse : ${(e as Error).message}`;
  }
